<script lang="ts">
	...
	renderer.link = function ({ href, title, text }) {
		switch (true) {
			...
			default:
				return `<a href="${href}" title="${title || ''}" onclick="location.reload();" class="mt-1.5 inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-2.5 py-1 text-sm text-primary transition-colors hover:bg-primary/20 [&>svg]:size-3" >
				${MessageCircle}
				${text}
				</a>
					`;
