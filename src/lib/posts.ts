...
async function parseMarkdownPost(content: string, threadId: string | null = null): Promise<Post> {
...
export async function getPostsWithThreads(): Promise<PostWithThread[]> {
	...
	return posts.map((post) => ({
		...post,
		thread: post.threadId ? threads.find((t) => t.id === post.threadId) : undefined
	}));
}
